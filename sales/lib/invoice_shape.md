...
| `lines[].sku` | **Solo si el catálogo está activo** (`sales_context` → `catalog.active`). Referencia un producto del catálogo: el servidor autorrellena descripción, precio de tarifa, tipo, unidad y clasificación contable. **Lo que TÚ mandes gana** (manda `unit_price` solo si el cliente dijo un importe distinto de la tarifa; los `taxes` del cliente también ganan). El sku sale de la búsqueda (`POST /api/products/actions/search_for_agents`) — **nunca lo inventes**. Sku desconocido → issue `PRODUCT_NOT_FOUND`. |
